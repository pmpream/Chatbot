...
const { connectToDatabase, getDataFromId, getDataFromBuildingName, getDataFromName, getDataFromLocality, getRandomDataFromDessert, 
  getRandomDataFromGrilled_shabu, getRandomDataFromMuslim, getRandomDataFromNoodles, getRandomDataFromPharmacy, getRandomDataFromRestaurant} = require('./database');
const { addConversationHistory } = require('./conversation');

const mysql = require('mysql');

const userId = 'line_user_id';
const channelAccessToken = '4eL6wA7UnqB0/LpyOl9yQ9JhkqEC1gNuVMiNKOMd5ondkD7VEKAj5oY599s2xzZgivKFx0VtAH0X1uk+ihTLSaYNCo2mr4HtF8wg7ITVrMDKJSOAUELPGVzSxQupQ/1eSWVrqrAQS6BUrarvyVGmGQdB04t89/1O/w1cDnyilFU=';
...
// เรียกใช้ /webhook เมื่อมีการ POST ข้อมูลเข้ามา
app.post('/webhook', async (req, res) => {
  try {
    const userId = req.body.originalDetectIntentRequest.payload.data.source.userId;
    const displayName = req.body.originalDetectIntentRequest.payload.data.source.displayName;

    const query = 'INSERT INTO conversation_history (line_user_id, display_name) VALUES (?, ?)';
    db.query(query, [userId, displayName], (err, result) => {
      if (err) {
        console.error('เกิดข้อผิดพลาดในการบันทึกข้อมูล: ', err);
        res.status(500).json({ error: 'Internal server error' });
      } else {
        console.log('บันทึกข้อมูล User ID และ Display Name แล้ว:', userId, displayName);
        res.sendStatus(200);
      }
    });
  } catch (error) {
